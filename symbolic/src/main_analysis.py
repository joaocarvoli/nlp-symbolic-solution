...
from utils.constants import SENTIMENT_COLUMN
...
if __name__ == '__main__':
    ...
    movies = movies[['movie_title', "comment", "rating_label"]]
    movies.rename(columns={"comment":"review_text", "rating_label":"polarity"}, inplace=True)
    SEED = 42
    movies_FT, movies_test = train_test_split(
        movies, test_size=2704/7704, stratify=movies['polarity'], random_state=SEED
    )

    inputs = tokenizer(
        movies_test['comment'].tolist(),
        truncation=True, 
        padding='max_length', 
        max_length=512, 
        return_tensors='pt'
    ...
    with torch.no_grad():
        ...
        movies[f'{SENTIMENT_COLUMN}_ia'] = results
        movies[f'{SENTIMENT_COLUMN}_ia'] = movies[f'{SENTIMENT_COLUMN}_ia'].replace(0, -1)
        
        overall_ia, positive_ia, negative_ia = evaluate_sentiment_results(movies, f'{SENTIMENT_COLUMN}_ia')
        movies['rating_label'] = (movies['numeric_rating'] >= 2.5).astype(int) ### mudei de 3.0 para 2.5 para ver como ficariam os resultados
        ...
        print(f"Overall accuracy of symbolic solution: {overall_ia:.2f}%")
        print(f"Positive review accuracy of symbolic solution: {positive_ia:.2f}%")
        print(f"Negative review accuracy of symbolic solution: {negative_ia:.2f}%")

        final_dict = {}

        for movie, group in movies.groupby('movie_title'):
            ...
            media_polarity = group['polarity'].mean()

            if media_polarity > 2.5:
                movie_sentiment = 1
            else:
                movie_sentiment = -1
            ...
            final_dict[movie] = {
                ...
                'polarity': round(media_polarity, 2),  # arredondado para 2 casas
